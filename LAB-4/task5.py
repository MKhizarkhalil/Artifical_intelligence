...
class ReflexModelBasedVaccumAgent(Agent):
    def __init__(self, environment):
        self.environment = environment
        self.model = {}  # Internal model to store the environment's state
    
    def sense(self, environment):
        # No sensing capability in the reflex-based model
        pass
    ...
    def act(self):
        ...
        if current_room.status == 'dirty':
            ...
            self.model[current_room.location] = 'clean'  # Update internal model
            return 'clean', 25
        
        # Check the model for the status of the current room
        if self.model.get(current_room.location, 'dirty') == 'dirty':
            # If the model says the room is dirty, clean it
            self.model[current_room.location] = 'clean'  # Update internal model
            return 'clean', 25
        
        # Move to the next room if it exists
        index = self.environment.rooms.index(current_room)
...
if __name__ == '__main__':
    ...
    env = NRoomVaccumCleanerEnvironment(None, room_statuses)
    vcagent = ReflexModelBasedVaccumAgent(env)  # Initialize agent with environment
    env.agent = vcagent  # Set environment's agent after agent initialization
    env.executeAll()
